...
export const getErrorMessage = {
  ...
  guestNickName: (guestNickName: string) => {
    ...
    if (guestNickName.length < GUEST_NICKNAME_MIN_LENGTH)
      return `이름은 최소 ${GUEST_NICKNAME_MIN_LENGTH} 글자 이상이어야 합니다.`;
  ...
  guestPassword: (guestPassword: string) => {
    ...
    if (guestPassword.length < GUEST_PASSWORD_MIN_LENGTH)
      return `비밀번호는 최소 ${GUEST_PASSWORD_MIN_LENGTH} 글자 이상이어야 합니다.`;
